...
/**
 * The out-patient bill (task 3.6, reshaped to Farhat's Medi-Pro layout), now printable from
 * anywhere that holds an invoice — the check-in slip AND the invoice reprint (task 6.1).
 *
 ...
 * and the hospital's own address, signature line and run time at the foot. The fields their
 * old system had but this one does not — token number, OPD card number, a separate
 * registration number — are simply left out rather than printed blank.
 *
 * NOT a `<header>`/`<footer>`: the print stylesheet hides those as page chrome (they are the
 * app's top bar and nav), which is exactly why the hospital name vanished on the first
 ...
 *
 * `visit` is optional because the invoice model does not require one; `receiptDate` is the
  ...
  return (
    <div className="receipt-bill space-y-3 text-sm text-foreground">
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-1 sm:grid-cols-3 md:grid-cols-6">
        ...
        <Cell label={t('reception.receipt.receiptDate')} value={dateTime(receiptDate)} />
...
function TotalLine({
  ...
  return (
    <div className="flex items-baseline justify-between">
      ...
      <dd className={strong ? 'font-mono text-base font-bold' : 'font-mono'} dir="ltr">
        {value} {currency}
      </dd>
    </div>
  )
